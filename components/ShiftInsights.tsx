...
export default function ShiftInsights({
  ...
  const currentCycleProgress = Math.min(
    100,
    (cyclesDoneNum / totalCycles) * 100,
  );
  ...
  const cards = [
    {
      label: "المغادرة",
      value:
        daysToFly === 0 ? "يوم السفر!" : <AnimatedValue value={daysToFly} />,
    ...
    },
    {
      ...
      value: (
        <div className="flex items-baseline gap-1">
          <AnimatedValue value={currentCycleProgress} toFixed={1} />%
        </div>
      ),
      sub:
        systemType === "3x8_industrial"
          ? `${cyclesDoneNum.toFixed(1)} دورة`
          : `${daysPassed} يوم`,
      icon: TrendingUp,
      color: "text-blue-400",
      ...
      value:
        systemType === "3x8_industrial" ? (
          ...
          <div className="flex gap-2">
            <div className="flex items-center gap-1.5">
              <Briefcase size={14} />
              {adminWorkDaysRemaining}
            </div>
            <div className="flex items-center gap-1.5">
              <Coffee size={14} />
              {adminRestDaysRemaining}
            </div>
          </div>
        ),
      sub:
      ...
      {cards.map((card, idx) => (
        ...
        >
          ...
          <div className="flex justify-between items-start z-10">
            <div
              className={`p-2 rounded-2xl bg-white/5 border border-white/10 ${card.color}`}
            >
          ...
          <div className="mt-4 z-10">
            <div
              className={`text-3xl lg:text-4xl font-black tracking-tighter ${card.color}`}
            >
              {card.value}
            </div>

...
export default function Onboarding({ onComplete }: OnboardingProps) {
      ...
      content: (
        <div className="flex flex-col items-center">
          <DatePickerAr 
            ...
            onChange={(date) => setFormData({ ...formData, cycleStartDate: format(date, 'yyyy-MM-dd') })}
          />
        </div>
      )
    },
      ...
      content: (
        <div className="flex flex-col gap-2">
          {[
            ...
            { id: 2, label: 'اليوم الثاني: صباحي + ليلي', desc: '07h-13h / 20h-07h' },
            { id: 3, label: 'اليوم الثالث: راحة', desc: 'ابتداءً من 07:00' },
          ].map((shift) => (
            <button
              key={shift.id}
              onClick={() => setFormData({ ...formData, initialCycleDay: shift.id })}
            ...
            >
              <div className={`font-black text-sm ${formData.initialCycleDay === shift.id ? 'text-white' : ''}`}>
                {shift.label}
              </div>
    ...
    },
    {
      ...
      content: (
        <div className="flex flex-col gap-3">
          {[
            { id: '3x8_industrial', label: 'نظام (3×8) الصناعي', desc: 'دورة من 3 أيام (مسائي، صباحي+ليلي، راحة)' },
            { id: '5x2_admin', label: 'نظام (5×2) الإداري', desc: 'دورة إسبوعية (أحد - خميس)' }
          ].map((sys) => (
            <button
              key={sys.id}
              onClick={() => setFormData({ ...formData, systemType: sys.id as SystemType })}
            ...
            >
              <div className="flex flex-col gap-1">
                <div className={`font-black text-sm ${formData.systemType === sys.id ? 'text-blue-400' : 'text-slate-200'}`}>
                  {sys.label}
                </div>
          ...
          <div className="p-4 rounded-2xl border border-dashed border-white/5 opacity-40 grayscale flex flex-col gap-1 text-right">
             <span className="text-xs font-black text-slate-500">أنظمة أخرى...</span>
             <span className="text-[9px] font-bold text-slate-600 italic">قريباً في التحديثات القادمة</span>
          </div>
        </div>
      )
    },
        ...
        <div className="flex flex-col gap-6">
          <div className="flex gap-4">
            <div className="flex-1 flex flex-col gap-2 text-center">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">أيام العمل</label>
              <input 
                type="number"
                value={formData.workDuration}
                onChange={(e) => setFormData({ ...formData, workDuration: parseInt(e.target.value) || 28 })}
            ...
            <div className="flex-1 flex flex-col gap-2 text-center">
              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">أيام الإجازة</label>
              <input 
                type="number"
                value={formData.vacationDuration}
                onChange={(e) => setFormData({ ...formData, vacationDuration: parseInt(e.target.value) || 7 })}
                className="w-full bg-white/[0.04] border border-white/[0.1] rounded-2xl px-4 py-3 text-lg font-black text-center text-white outline-none focus:border-blue-500 transition-all font-Tajawal"
              />
            </div>
          </div>
          ...
          <button
            onClick={() => setFormData({ ...formData, addRouteDays: !formData.addRouteDays })}
          ...
          >
            <div className="text-right">
              <div className={`font-black text-sm ${formData.addRouteDays ? 'text-white' : ''}`}>أيام الطريق (Route Days)</div>
              <div className="text-[10px] opacity-60 mt-0.5">إضافة +2 يوم لإجمالي الإجازة</div>
            </div>
            <div className={`w-10 h-6 rounded-full relative transition-colors ${formData.addRouteDays ? 'bg-blue-600' : 'bg-white/10'}`}>
              <motion.div 
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-[#020617] flex flex-col items-center justify-start overflow-y-auto p-6 text-right" dir="rtl">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <GlassCard className="p-6 md:p-8 flex flex-col gap-6 md:gap-8 relative overflow-visible" glow glowColor="#3b82f6">
            <div className="flex flex-col gap-2">
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-black text-blue-500 uppercase tracking-[0.3em]">الخطوة {step} من {steps.length}</span>
              </div>
              <h1 className="text-2xl md:text-3xl font-black text-slate-100 italic">{currentStep.title}</h1>
              <p className="text-xs md:text-sm font-black text-slate-500">{currentStep.description}</p>
            </div>

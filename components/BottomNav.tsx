...
export type NavTab = 'HOME' | 'AGENDA' | 'STATS' | 'PROFILE';
...
export default function BottomNav({ activeTab, onTabChange }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 h-[88px] bg-[#020617]/80 backdrop-blur-2xl border-t border-white/[0.08] px-6 pb-6 pt-2 z-50 flex justify-between items-center" dir="rtl">
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            <div className={`relative z-10 ${isActive ? 'text-white' : 'text-slate-500'}`}>
              {tab.icon}
            </div>
